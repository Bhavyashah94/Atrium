...
final beszelChartTimeProvider = StateProvider<ChartTime>(
  (ref) => ChartTime.hour1,
);
  ...
  if (instance.auth is InstanceAuthUserPass) {
    ...
    try {
      final loginResp = await dio.post<Map<String, dynamic>>(
        ...
        data: {'identity': creds.username, 'password': creds.password},
      );
...
final beszelSystemStatsProvider = FutureProvider.autoDispose
    .family<
      List<BeszelStats>,
      ({Instance instance, String systemId, ChartTime chartTime})
    >((Ref ref, args) async {
      ref.pollEvery(_getChartPollInterval(args.chartTime));
      final BeszelApi api = await ref.watch(beszelApiProvider(args.instance));
      final systemStats = await api.getSystemStats(
        args.systemId,
        args.chartTime,
      );
      final containerStatsRaw = await api.getAllContainerStats(
        args.systemId,
        args.chartTime,
      );
      // Parse container stats into a structured list
      final parsedContainerStats =
          <
            ({
              int createdTimeMs,
              double cpu,
      ...
      for (final data in containerStatsRaw) {
        ...
        for (final containerStat in statsList) {
          ...
          if (s['b'] != null) {
            ...
            totalNetSent +=
                ((s['ns'] as num?)?.toDouble() ?? 0.0) * 1024 * 1024;
            totalNetRecv +=
                ((s['nr'] as num?)?.toDouble() ?? 0.0) * 1024 * 1024;
          }
        }
