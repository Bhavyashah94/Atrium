  ...
  CpuHistoryState build() {
    ref.listen(dashdotCpuLoadProvider(instance), (previous, next) {
      if (next.hasValue && next.value != null && next.value is List) {
        ...
        if (cores.isNotEmpty) {
          double totalLoad = 0;
          final List<double> coreLoads = [];
          final List<double> coreTemps = [];
          ...
          final List<double> newOverall = List.from(state.overall.values)
            ..add(avgLoad);
          if (newOverall.length > _maxHistoryPoints) newOverall.removeAt(0);

          final List<MetricHistory> newCores = [];
          for (int i = 0; i < coreLoads.length; i++) {
            final List<double> prevCoreValues = state.cores.length > i
                ? state.cores[i].values
                : [];
            final List<double> prevCoreTemps = state.cores.length > i
                ? state.cores[i].temps
                : [];
            final List<double> newCore = List.from(prevCoreValues)
              ..add(coreLoads[i]);
            final List<double> newTemps = List.from(prevCoreTemps)
              ..add(coreTemps[i]);
...
final dashdotCpuHistoryProvider =
    NotifierProvider.family<CpuHistoryNotifier, CpuHistoryState, Instance>(
  ...
  MetricHistory build() {
    ref.listen(dashdotRamLoadProvider(instance), (previous, next) {
      if (next.hasValue && next.value != null && next.value is Map) {
        ...
        final List<double> newValues = List.from(state.values)
          ..add(loadGb.toDouble());
...
final dashdotRamHistoryProvider =
    NotifierProvider.family<RamHistoryNotifier, MetricHistory, Instance>(
...
final dashdotStorageHistoryProvider =
    NotifierProvider.family<StorageHistoryNotifier, MetricHistory, Instance>(
  ...
  NetworkHistory build() {
    ref.listen(dashdotNetworkLoadProvider(instance), (previous, next) {
      if (next.hasValue && next.value != null && next.value is Map) {
        ...
        final List<double> newDown = List.from(state.down)
          ..add(down.toDouble());
...
final dashdotNetworkHistoryProvider =
    NotifierProvider.family<NetworkHistoryNotifier, NetworkHistory, Instance>(
    ...
    ref.listen(dashdotGpuLoadProvider(instance), (previous, next) {
      if (next.hasValue && next.value != null && next.value is Map) {
        ...
        if (layout.isNotEmpty) {
          double totalLoad = 0;
          for (final gpu in layout) {
            totalLoad += (gpu['load'] as num?)?.toDouble() ?? 0.0;
          }
          final avgLoad = totalLoad / layout.length;

          final List<double> newValues = List.from(state.history.values)
            ..add(avgLoad);
          if (newValues.length > _maxHistoryPoints) {
            newValues.removeAt(0);
          }
...
final dashdotGpuHistoryProvider =
    NotifierProvider.family<GpuHistoryNotifier, GpuHistoryState, Instance>(
