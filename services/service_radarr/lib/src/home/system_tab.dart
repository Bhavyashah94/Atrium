...
class _SystemTabState extends ConsumerState<SystemTab> {
  ...
  Future<void> _confirmRestart() async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext ctx) => AlertDialog(
        title: const Text('Restart Radarr?'),
        content: const Text(
          'This will restart the Radarr service. It will be temporarily unavailable.',
        ),
        actions: <Widget>[
          TextButton(
  ...
  Future<void> _confirmShutdown() async {
    final bool? confirmed = await showDialog<bool>(
      context: context,
      builder: (BuildContext ctx) => AlertDialog(
        title: const Text('Shutdown Radarr?'),
        content: const Text(
          'This will shut down the Radarr service. You will need to restart it manually.',
        ),
        actions: <Widget>[
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
...
class _StatusTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final ThemeData theme = Theme.of(context);
    final statusAsync = ref.watch(radarrSystemStatusProvider(instance));
    ...
    return EasyRefresh(
      ...
      onRefresh: () async {
        ...
        await Future.wait(<Future<void>>[
          ref.read(radarrSystemStatusProvider(instance).future),
          ref.read(radarrHealthProvider(instance).future),
          ref.read(radarrDiskSpaceProvider(instance).future),
        ]);
      },
      child: ListView(
